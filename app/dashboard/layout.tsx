...
export default async function DashboardLayout({
  ...
  const { data: profile } = await supabase
    .from("profiles")
    .select("full_name, credits, role, user_type, conveyancer_verified")
    .eq("id", user.id)
    .single();
  ...
  // Show CRM nav only for verified conveyancers and lawyers
      ...
      <header className="bg-white border-b border-gray-200 h-14 flex items-center px-6 gap-4 flex-shrink-0 relative">
        ...
        <div className="flex items-center gap-3">
          ...
          {profile?.role === "admin" && (
            <Link
              href="/admin"
              className="text-xs text-gray-500 hover:text-gray-900 font-medium px-2 py-1 rounded border border-gray-200 hover:bg-gray-50 transition-colors"
            >
              Admin
            </Link>
          )}
