...
export default function DashboardPage() {
  ...
  async function load() {
    ...
    // Count draft vs finalised reports
    ...
    <div className="space-y-8 pb-10">
      ...
      <div className="flex items-start justify-between">
        <div>
          ...
          <p className="text-sm text-gray-500 mt-1">{typeLabel} Dashboard · {total} customer{total !== 1 ? 's' : ''} in your CRM</p>
        </div>
      ...
      {/* Stats — replaced "Clients Invited" with "Tasks Pending" */}
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
        <div className="bg-white rounded-xl border border-gray-200 p-5 shadow-sm">
          <span className="text-2xl">👥</span>
          <p className="text-3xl font-bold leading-none mt-3 text-gray-900">{total}</p>
          <p className="text-sm font-semibold text-gray-700 mt-1.5">Total Customers</p>
        ...
        <div className="bg-white rounded-xl border border-gray-200 p-5 shadow-sm">
          <span className="text-2xl">📌</span>
          <p className={`text-3xl font-bold leading-none mt-3 ${tasksPending > 0 ? 'text-orange-600' : 'text-gray-400'}`}>{tasksPending}</p>
          <p className="text-sm font-semibold text-gray-700 mt-1.5">Tasks Pending</p>
          <p className="text-xs text-gray-400 mt-0.5">Across all properties</p>
        </div>
      </div>
      ...
      <div>
        ...
        {loading ? (
          ...
          <div className="bg-white rounded-xl border border-dashed border-gray-300 p-10 text-center">
            ...
            <p className="text-xs text-gray-400 mb-3">{search ? 'Try a different search' : 'Add your first customer to get started'}</p>
            {!search && (
              <Link href="/dashboard/customers/new" className="inline-flex items-center gap-2 bg-[#E8001D] text-white px-5 py-2.5 rounded-lg font-semibold text-sm hover:bg-red-700 transition-colors">+ Add Customer</Link>
            )}
          </div>
        ) : (
            ...
            {filtered.slice(0, 8).map(c => {
              ...
              return (
                <Link key={c.id} href={`/dashboard/customers/${c.id}`}
                  className="flex items-center justify-between bg-white rounded-xl border border-gray-200 p-4 hover:shadow-md hover:border-gray-300 transition-all group">
                  <div className="flex items-center gap-3">
                    <div className="w-10 h-10 rounded-full bg-red-50 border-2 border-red-100 flex items-center justify-center text-[#E8001D] font-bold text-sm">
                  ...
                  <div className="flex items-center gap-3">
                    ...
                    <span className={`inline-flex items-center gap-1.5 text-xs font-bold px-2.5 py-1 rounded-full border ${s.badgeBg} ${s.textColor}`}>
